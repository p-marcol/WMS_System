...
\begin{itemize}
    ...
    \item \texttt{JwtTokenUtils} - zawierająca metody do generowania, weryfikacji i wyłuskiwania danych z tokenów JWT (ang. \english{JSON Web Token}).
\end{itemize}
...
Odczytanie harmonogramu użytkownika jest dużo bardziej złożone niż jednostki organizacyjnej. Wymaga ono odczytu wszystkich jednostek, do których należał użytkownik w danym okresie czasu, następnie dla każdej z nich odczytania harmonogramu i połączenia ich w jeden widok przechowywany w klasie pomocniczej \texttt{Calendar}. Jeżeli użytkownik posiada również własny harmonogram, serwer generuje jego widok i nadpisuje dane przechowywane w klasie. Diagram sekwencji procesu został przedstawiony na rysunku \ref{fig:scheduleSequence}. \newpage
...
\begin{itemize}
    ...
    \item Dioda LED RGB (ang. \english{Red, Green, Blue}) ze wspólną katodą, sygnalizująca stan czytnika.
\end{itemize}
